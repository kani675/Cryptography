# ================= AES SBOX =================
AES_SBOX = [

]

RCON = [0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36]

# ================= UTILITIES =================
def hex_to_bytes(s):
    s = s.replace(" ","")
    return [int(s[i:i+2],16) for i in range(0,len(s),2)]

def bytes_to_hex(arr):
    return ''.join(format(b,'02x') for b in arr)

# ================= GALOIS MULT =================
def galois_mult(a,b):
    p = 0
    for _ in range(8):
        if b & 1:
            p ^= a
        hi = a & 0x80
        a <<= 1
        if hi:
            a ^= 0x1b
        b >>= 1
    return p & 0xff

# ================= KEY EXPANSION =================
def key_expansion(key):
    w=[]
    for i in range(4):
        w.append(key[4*i:4*i+4])

    for i in range(4,44):
        temp=w[i-1].copy()
        if i%4==0:
            temp=temp[1:]+temp[:1]
            temp=[AES_SBOX[b] for b in temp]
            temp[0]^=RCON[(i//4)-1]

        word=[w[i-4][j]^temp[j] for j in range(4)]
        w.append(word)

    return w

# ================= STATE FUNCTIONS =================
def bytes_to_state(b):
    s=[[0]*4 for _ in range(4)]
    for c in range(4):
        for r in range(4):
            s[r][c]=b[c*4+r]
    return s

def state_to_bytes(s):
    out=[]
    for c in range(4):
        for r in range(4):
            out.append(s[r][c])
    return out

def sub_bytes(state):
    return [[AES_SBOX[b] for b in row] for row in state]

def shift_rows(state):
    s=[row[:] for row in state]
    for r in range(1,4):
        s[r]=s[r][r:]+s[r][:r]
    return s

def mix_columns(state):
    s=[row[:] for row in state]
    for c in range(4):
        a=[s[0][c],s[1][c],s[2][c],s[3][c]]

        s[0][c]=galois_mult(2,a[0])^galois_mult(3,a[1])^a[2]^a[3]
        s[1][c]=a[0]^galois_mult(2,a[1])^galois_mult(3,a[2])^a[3]
        s[2][c]=a[0]^a[1]^galois_mult(2,a[2])^galois_mult(3,a[3])
        s[3][c]=galois_mult(3,a[0])^a[1]^a[2]^galois_mult(2,a[3])

    return s

def add_round_key(state,w,round):
    s=[row[:] for row in state]
    for c in range(4):
        for r in range(4):
            s[r][c]^=w[round*4+c][r]
    return s

# ================= AES ENCRYPT =================
def aes_encrypt(plaintext,key):

    pt_bytes=hex_to_bytes(plaintext)
    key_bytes=hex_to_bytes(key)

    w=key_expansion(key_bytes)

    state=bytes_to_state(pt_bytes)

    print("Initial AddRoundKey")
    state=add_round_key(state,w,0)

    for round in range(1,10):

        print("\nRound",round)

        state=sub_bytes(state)
        state=shift_rows(state)
        state=mix_columns(state)
        state=add_round_key(state,w,round)

        print("State:",bytes_to_hex(state_to_bytes(state)))

    print("\nFinal Round")

    state=sub_bytes(state)
    state=shift_rows(state)
    state=add_round_key(state,w,10)

    ciphertext=bytes_to_hex(state_to_bytes(state))

    return ciphertext


# ================= TEST =================
plaintext="2b7e151628aed2a6abf7158809cf4f3c"
key="2b7e151628aed2a6abf7158809cf4f3c"

print("Ciphertext:",aes_encrypt(plaintext,key))